...
class DiscordTracker {
  ...
  async performSearch() {
    ...
    try {
      ...
      try {
        const externalResponse = await fetch(`${this.apiBaseUrl}/proxy/user/${query}`, {
          credentials: 'include'
        });
        if (externalResponse.ok) {
          ...
          if (externalData && externalData.success && externalData.data && externalData.data.user) {
            ...
            console.log('✅ User found via external API proxy:', userData);
          }
        } else {
          console.warn('⚠️ External proxy responded with status:', externalResponse.status);
        }
      } catch (externalError) {
        console.log('⚠️ External API proxy not available, trying internal API');
      }
  ...
  async loadTrackingList() {
    try {
      ...
      if (response.ok) {
        ...
        console.log('[DEBUG] trackingList loaded:', this.trackingList);
        ...
        // Render the updated tracking list
