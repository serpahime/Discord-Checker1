...
class DiscordTracker {
  ...
  renderTrackingList(list) {
    ...
    trackingList.innerHTML = list.map(user => {
      ...
      const name = user.username || user.displayName || user.globalName || `User_${user.userId}`;
  ...
  async loadCompleteUserDataForTracking() {
    ...
    for (let i = 0; i < this.trackingList.length; i++) {
      ...
      // Check if user needs complete data
      if (trackedUser.userId && (!trackedUser.username || trackedUser.username === 'Unknown User' || !trackedUser.avatar)) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (data.success && data.data && data.data.user) {
              ...
              console.log('[DEBUG] Updated user data:', this.trackingList[i]);
            }
          }
        } catch (error) {
    ...
    // Re-render the tracking list with updated data
    this.renderTrackingList(this.trackingList);
  }
