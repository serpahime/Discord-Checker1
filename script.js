...
class DiscordTracker {
  constructor() {
    ...
    this.isTrackingUser = false; // Flag to prevent duplicate tracking requests
    this.userProfile = null;
    this.trackingList = [];
    this.apiBaseUrl = 'https://fishonxd.su/api/checks/';
  ...
  bindEvents() {
    ...
    document.querySelectorAll('.menu-item').forEach(item => {
      item.addEventListener('click', (e) => {
        ...
        this.navigateToSection(action).catch(error => {
    ...
    document.addEventListener('keydown', (e) => {
      if (e.ctrlKey && e.key === 'k') {
        e.preventDefault();
        this.navigateToSection('search').catch(error => {
          console.error('Navigation error:', error);
        });
      }
    });
  ...
  async navigateToSection(section) {
    ...
    try {
      ...
      switch (section) {
        ...
        case 'tracking':
          ...
          if (trackingManagement) {
            ...
            // Force reload complete user data
      ...
      console.error('❌ Navigation error:', error);
      // Fallback to home
      this.navigateToSection('home').catch(error => {
        console.error('Navigation error:', error);
      });
    }
  }
  ...
  ensureProperNavigationState() {
    ...
    if (!visibleSection || sections.filter(id => {
      ...
      if (this.isAuthenticated) {
        ...
        this.navigateToSection(savedSection).catch(error => {
          console.error('Navigation error:', error);
        });
      } else {
        this.navigateToSection('home').catch(error => {
  ...
  async checkAuthStatus() {
    // Backend auth disabled; treat as authenticated UI
  ...
  async performSearch() {
    ...
    if (this.isLoading) {
      this.showNotification('Поиск уже выполняется, пожалуйста, подождите', 'info');
      return;
    }
    ...
    try {
      ...
      // First try to get user from external API via our proxy
      let userData = null;
      let source = 'external';
      
      try {
        const externalResponse = await fetch(`${this.apiBaseUrl}user/${query}`);
        if (externalResponse.ok) {
          const externalData = await externalResponse.json().catch(() => null);
          if (externalData && externalData.success && externalData.data && externalData.data.user) {
            userData = externalData.data;
            console.log('✅ User found via external API:', userData);
          }
        } else {
          console.warn('⚠️ External responded with status:', externalResponse.status);
        }
      } catch (externalError) {
        console.log('⚠️ External API not available');
      }
  ...
  displayEnhancedUserProfile(userData, source) {
    ...
    if (this.currentUser.discordId || this.currentUser.id) {
      const id = this.currentUser.discordId || this.currentUser.id;
      this.fetchAndDisplayReputation(id);
      this.fetchAndDisplayViews(id);
      this.autoAddView(id);
    }
  }
  ...
  updateEnhancedProfileInfo(userData) {
    ...
    if (userAvatarProfile) {
      if (user.displayAvatarURL) {
        userAvatarProfile.src = user.displayAvatarURL;
      } else if (user.id && user.avatar) {
        userAvatarProfile.src = `https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}.webp`;
      } else {
        userAvatarProfile.src = 'https://cdn.discordapp.com/embed/avatars/0.png';
      }
    }
  ...
  async trackUser() {
    ...
    // Prevent multiple simultaneous track requests
    if (this.isTrackingUser) {
      this.showNotification('Добавление в отслеживание уже выполняется', 'info');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Update tracking list immediately to prevent duplicates
        ...
        // Reload tracking list to get fresh data from server
        await this.loadTrackingList();
      } else {
  ...
  async showDashboard() {
    await this.navigateToSection('home');
  }
  ...
  async showSearch() {
    await this.navigateToSection('search');
  }
  ...
  async showArchive() {
    await this.navigateToSection('archive');
  }

  async showServers() {
    await this.navigateToSection('servers');
  }
  ...
  async forceRefreshTrackingData() {
    console.log('[DEBUG] Force refreshing tracking data...');
    try {
      // Reload tracking list from server (this will also remove duplicates)
      await this.loadTrackingList();
      
      // Force reload complete user data for all users
      await this.loadCompleteUserDataForTracking();
      
      this.showNotification('Данные отслеживания обновлены', 'success');
    } catch (error) {
      console.error('[DEBUG] Error force refreshing tracking data:', error);
  ...
  async cleanupDuplicateEntries() {
    console.log('[DEBUG] Cleaning up duplicate entries...');
    try {
      // This will trigger the server-side cleanup
      await this.loadTrackingList();
      this.showNotification('Дублирующиеся записи удалены', 'success');
    } catch (error) {
      console.error('[DEBUG] Error cleaning up duplicates:', error);
      this.showNotification('Ошибка при очистке дублирующихся записей', 'error');
    }
  }
  ...
  renderTrackingList(list) {
    ...
    trackingList.innerHTML = list.map(user => {
      ...
      // Get proper avatar URL with better fallback logic
      ...
      // Get proper name with better fallback logic
      let name = user.username || user.displayName || user.globalName;
      
      // If still no name, try to get it from the user ID or use a better fallback
      if (!name || name === 'Unknown User' || name === 'User_' + user.userId) {
        // Try to extract a better name from the user ID or use a more descriptive fallback
        name = `User_${user.userId.slice(-6)}`; // Use last 6 digits for better identification
      }
      
      // If we still have "Unknown User", try to get a better name from the discriminator
      if (name === 'Unknown User' && user.discriminator && user.discriminator !== '0000') {
        name = `User#${user.discriminator}`;
      }
      
      // Get proper discriminator/tag
      let tag = '';
  ...
  updateTrackButton(user) {
    ...
    newTrackBtn.innerHTML = '<i class="fas fa-ban"></i> Трекинг отключён';
    newTrackBtn.className = 'btn btn-secondary';
    newTrackBtn.disabled = true;
  }

  // Remove user from tracking
  async removeFromTracking(discordId) {
    this.showNotification('Трекинг отключён', 'info');
  }
  ...
  async loadUserProfileData(discordId) {
    try {
      ...
      // Получаем через наш бэкенд-прокси, чтобы избежать CORS и унифицировать формат
      let response = await fetch(`${this.apiBaseUrl}/proxy/user/${discordId}`, { credentials: 'include' });
      let userData = null;
      if (response.ok) {
        const responseData = await response.json().catch(() => null);
        if (responseData && responseData.success) {
          userData = responseData.data || responseData;
          console.log('[DEBUG] User profile data loaded from proxy:', userData);
        }
      } else {
        console.warn('[WARN] Could not load user data from proxy, status:', response.status);
  ...
  async loadCompleteUserDataForTracking() {
    ...
    for (let i = 0; i < this.trackingList.length; i++) {
      const trackedUser = this.trackingList[i];
      console.log('[DEBUG] Checking user:', trackedUser);
      
      // Check if user needs complete data (more comprehensive check)
      const needsUpdate = trackedUser.userId && (
        !trackedUser.username || 
        trackedUser.username === 'Unknown User' || 
        trackedUser.username === 'User_' + trackedUser.userId ||
        !trackedUser.avatar || 
        trackedUser.avatar === 'https://cdn.discordapp.com/embed/avatars/0.png' ||
        !trackedUser.discriminator ||
        trackedUser.discriminator === '0000'
      );
      
      if (needsUpdate) {
        console.log('[DEBUG] Loading data for user:', trackedUser.userId);
        
        try {
          // Try to get user data from our search API
          const response = await fetch(`${this.apiBaseUrl}/search/${trackedUser.userId}`, {
            credentials: 'include'
          });
          
          if (response.ok) {
            const data = await response.json();
            console.log('[DEBUG] Search API response for', trackedUser.userId, ':', data);
            
            if (data.success && data.data && data.data.user) {
              const userData = data.data.user;
              
              // Update the tracking list with complete data
              const updatedUser = {
                ...trackedUser,
                username: userData.username || userData.displayName || trackedUser.username,
                displayName: userData.displayName || userData.username,
                avatar: userData.avatar || userData.displayAvatarURL || trackedUser.avatar,
                discriminator: userData.discriminator || trackedUser.discriminator,
                status: userData.status || 'offline',
                activity: userData.activity || trackedUser.activity,
                accountAge: userData.accountAge || trackedUser.accountAge,
                riskScore: userData.riskScore || trackedUser.riskScore || 0,
                bio: userData.bio || trackedUser.bio,
                nitro: userData.nitro || trackedUser.nitro,
                badges: userData.badges || trackedUser.badges || []
              };
              
              this.trackingList[i] = updatedUser;
              hasUpdates = true;
              
              console.log('[DEBUG] Updated user data:', updatedUser);
            } else {
              console.log('[DEBUG] No user data found for:', trackedUser.userId, 'Response:', data);
              
              // Try to get data from TrackedUser collection
              try {
                const trackedResponse = await fetch(`${this.apiBaseUrl}/tracked-user/${trackedUser.userId}`, {
                  credentials: 'include'
                });
                
                if (trackedResponse.ok) {
                  const trackedData = await trackedResponse.json();
                  console.log('[DEBUG] TrackedUser API response:', trackedData);
                  
                  if (trackedData.success && trackedData.data) {
                    const userData = trackedData.data;
                    ...
                    console.log('[DEBUG] Updated user data from TrackedUser:', updatedUser);
                  }
                }
              } catch (trackedError) {
                console.error('[DEBUG] Error loading from TrackedUser:', trackedError);
              }
            }
          } else {
            console.log('[DEBUG] Failed to fetch user data for:', trackedUser.userId, 'Status:', response.status);
          }
        } catch (error) {
          console.error('[DEBUG] Error loading user data:', error);
        }
      } else {
        console.log('[DEBUG] Skipping user, already has complete data:', trackedUser.username);
      }
    }
    
    // Only re-render if we have updates
    if (hasUpdates) {
      console.log('[DEBUG] Re-rendering tracking list with updated data');
...
async function showProfile() {
  await window.discordTracker.navigateToSection('search');
}
...
async function showTracking() {
  await window.discordTracker.navigateToSection('tracking');
}

async function showDashboard() {
  await window.discordTracker.navigateToSection('home');
}
