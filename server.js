...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      connectSrc: ["'self'", "https://fishonxd.su", "https://discord.com"],
...
// Отдаём статику из корня; дополнительные миддлвары для файлов не требуются
...
const sessionConfig = {
  ...
  cookie: {
    ...
    maxAge: 24 * 60 * 60 * 1000, // Reduced to 1 day for security
    httpOnly: true,
    sameSite: process.env.NODE_ENV === 'production' ? 'strict' : 'lax',
    domain: process.env.NODE_ENV === 'production' ? '.ds-chekcer-1.onrender.com' : undefined
  },
...
// Use MongoDB store for sessions in production, MemoryStore for development
...
try {
  ...
  app.get('/api/user/tracking', ensureAuthenticated, async (req, res) => {
    try {
      ...
      // Remove duplicates from tracking list
      const uniqueTrackingList = [];
      const seenUserIds = new Set();
      
      for (const item of user.trackingList) {
        if (!seenUserIds.has(item.userId)) {
          seenUserIds.add(item.userId);
          uniqueTrackingList.push(item);
        }
      }
      
      // Update user's tracking list if duplicates were found
      if (uniqueTrackingList.length !== user.trackingList.length) {
        console.log(`🧹 Removed ${user.trackingList.length - uniqueTrackingList.length} duplicate entries for user ${req.user.id}`);
        user.trackingList = uniqueTrackingList;
        await user.save();
      }
      
      res.json({ success: true, tracking: uniqueTrackingList });
    } catch (error) {
  ...
  app.post('/api/user/track', ensureAuthenticated, async (req, res) => {
    try {
      ...
      // Check if already tracking with more robust check
      const existing = user.trackingList.find(item => item.userId === discordId);
      if (existing) {
        console.log('⚠️ User already being tracked:', discordId);
        return res.status(400).json({ success: false, error: 'User already being tracked' });
      }
      
      // Create new tracking entry with better data preservation
      const newTrackingEntry = {
        ...
        discriminator: req.body.discriminator || '0000',
      ...
      user.trackingList.push(newTrackingEntry);
      
      await user.save();
      
      console.log('✅ User added to tracking:', {
        userId: req.user.id,
        trackedUserId: discordId,
        username: username
      });
  ...
  // Get tracked user data from TrackedUser collection
  app.get('/api/tracked-user/:discordId', ensureAuthenticated, async (req, res) => {
    try {
      const { discordId } = req.params;
      ...
      if (!trackedUser) {
        return res.status(404).json({ success: false, error: 'User not found in tracking database' });
      }
      
      res.json({ success: true, data: trackedUser });
    } catch (error) {
      console.error('❌ Get tracked user error:', error);
      res.status(500).json({ success: false, error: error.message });
    }
  });

  app.get('/api/search/:discordId', searchLimiter, ensureAuthenticated, async (req, res) => {
    try {
      // Check MongoDB connection
      if (mongoose.connection.readyState !== 1) {
      ...
      if (!trackedUser) {
        try {
          ...
          // Fetch from external API with retry logic and improved headers
          const apiHeaders = {
          ...
          if (process.env.FISHONXD_API_KEY) {
            apiHeaders['Authorization'] = `Bearer ${process.env.FISHONXD_API_KEY}`;
          }
          ...
          if (apiError.response?.status === 403) {
            console.warn('⚠️ API Access Denied (403) - Check API key and permissions');
          } else if (apiError.response?.status === 429) {
            console.warn('⚠️ API Rate Limited (429) - Too many requests');
          } else if (apiError.response?.status === 404) {
            console.warn('⚠️ User not found in external API (404)');
          }
  ...
  // --- USER PROXY ROUTE (External API proxy to avoid CORS) ---
  app.get('/api/proxy/user/:discordId', async (req, res) => {
    try {
      const { discordId } = req.params;
      console.log(`🌐 Attempting to fetch user data from external API for ID: ${discordId}`);
      ...
      if (response.data && response.data.success) {
        console.log(`✅ External API user data loaded successfully for ID: ${discordId}`);
        res.json(response.data);
      } else {
        throw new Error('Invalid response format from external API');
      }
      ...
      console.log(`⚠️ External API unavailable for user ID ${req.params.discordId}, error:`, error.message);
      
      // Return error response that frontend can handle
      res.status(404).json({
        success: false,
        error: 'User not found in external API',
        message: 'User data could not be retrieved from external source'
      });
    }
  });
