...
// Статическая раздача отключена — фронтенд удалён
...
try {
  app.get('/', (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: 'Frontend removed. Use external API at https://fishonxd.su/api/checks',
        externalApi: 'https://fishonxd.su/api/checks'
      });
    } catch (error) {
  ...
  app.get('/dashboard', (req, res) => {
    try {
      ...
      res.status(410).json({ success: false, error: 'Frontend removed' });
    } catch (error) {
...
try {
  // Disable auth endpoints — frontend removed
  app.get('/auth/discord', (req, res) => res.status(410).json({ success: false, error: 'Auth disabled' }));

  app.get('/auth/discord/callback', (req, res) => res.status(410).json({ success: false, error: 'Auth disabled' }));

  app.get('/auth/logout', (req, res) => res.status(410).json({ success: false, error: 'Auth disabled' }));
...
try {
  ...
  app.post('/api/user/track', async (req, res) => res.status(410).json({ success: false, error: 'Tracking feature has been removed' }));

  app.delete('/api/user/untrack/:discordId', async (req, res) => res.status(410).json({ success: false, error: 'Tracking feature has been removed' }));
  ...
  app.post('/api/user/track/note', async (req, res) => {
    // Tracking system disabled
    return res.status(410).json({ success: false, error: 'Tracking feature has been removed' });
  });

  // Tracking data endpoint disabled
  app.get('/api/tracked-user/:discordId', async (req, res) => {
    return res.status(410).json({ success: false, error: 'Tracking feature has been removed' });
  });

  app.get('/api/search/:discordId', searchLimiter, async (req, res) => {
    try {
      // Check MongoDB connection
      if (mongoose.connection.readyState !== 1) {
  ...
  app.get('/api/user/:discordId', ensureAuthenticated, async (req, res) => {
    try {
      const { discordId } = req.params;
      // Tracking feature removed: return 404 for user details by Discord ID
      return res.status(404).json({ success: false, error: 'User not found' });
    } catch (error) {
  ...
  // Tracking priority endpoint disabled
